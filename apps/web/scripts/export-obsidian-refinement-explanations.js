...
async function main() {
  ...
  const sourceMarkdownPath = join(
    ...
    "refinement-explanations.md",
  );
  const summaryMarkdown = await readFile(sourceMarkdownPath, "utf-8");
  const recordMarkdown = buildObsidianRefinementExplanationSummaryRecord({
    generatedDate,
    sourceMarkdownPath,
    summaryMarkdown,
  });
  const targetDir = join(obsidianRoot, "05_验证与实验", "二轮解释验证记录汇总", "已生成记录");
  const targetPath = join(targetDir, `二轮解释验证汇总_${generatedDate}.md`);
