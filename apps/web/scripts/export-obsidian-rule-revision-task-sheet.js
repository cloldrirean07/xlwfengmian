...
async function main() {
  ...
  const recordMarkdown = buildObsidianRuleRevisionTaskSheetRecord({
    generatedDate,
    sourceMarkdownPath,
    summaryMarkdown,
  });
  const targetDir = join(obsidianRoot, "05_验证与实验", "规则修订任务单", "已生成记录");
  const targetPath = join(targetDir, `规则修订任务单_${generatedDate}.md`);
